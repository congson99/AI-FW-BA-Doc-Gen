...
8. Check for existing downstream documents in `workspace/<folder-name>/`:
   - Look for: `business_rule_<slug>.md`, `ba_doc_<slug>.md`
...
3. Create `workspace/<folder-name>/ac_<slug>.md` using the format defined in `framework/styles/style_ac.md`.

4. Confirm:
```
✓ workspace/<folder-name>/ac_<slug>.md
